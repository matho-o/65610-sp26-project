...
# ── repack test ───────────────────────────────────────────────────────────
if tests_to_run[3]:
    print("\n--- Step 6: Testing matrix.repack ---")
    try:
        # from size = 16 down to k = 4
        res_repack = matrix.repack(cipher_a, size, k, cryptocontext, key_pair.publicKey)

        dec_repack = cryptocontext.Decrypt(res_repack, key_pair.secretKey)
        dec_repack.SetLength(k * k)
        actual_repack = [x.real for x in dec_repack.GetCKKSPackedValue()]

        # Calculate Expected
        # top left k * k matrix in matrix_a_data
        np_a = np.array(matrix_a_data).reshape(size, size)
        expected_repack = np_a[:k, :k].flatten().tolist()

        print(f"Expected: {expected_repack}")
        print(f"Actual:   {actual_repack}")

        if np.allclose(actual_repack, expected_repack, atol=0.001):
